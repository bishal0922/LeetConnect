...
const Verify = () => {
  ...
  const fetchUserToken = async (email) => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/user/get-user-token`, {
        params: { email },
      });
      ...
      if (response.data.success) {
        ...
        setSnackbarMessage(
          "Failed to link LeetCode account. Please try again.",
        );
      ...
      setSnackbarMessage(
        "An error occurred while linking LeetCode account. Please try again later.",
      );
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
    }
  ...
  // Handles the "Verify" button when linking the leetcode account
  const handleVerify = async () => {
    try {
      ...
      setSnackbarMessage(
        "An error occurred while verifying token. Please try again later.",
      );
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
    }
    ...
    <Container>
      <Box sx={{ my: 4 }}>
        ...
        <Button
          variant="contained"
          color="primary"
          onClick={() => fetchUserToken(user.email)}
          sx={{ mt: 2 }}
        >
